...
//                 resizeTo: hostEl, // Let the canvas resize with the container
//             });
...
//             // Use renderer dimensions instead of host element dimensions
//             const rendererWidth = renderer.width;
//             const rendererHeight = renderer.height;

//             const aspectRatio = manTex.width / manTex.height;
//             const targetHeight = rendererHeight * 0.8; // Use 80% of renderer height
//             const targetWidth = targetHeight * aspectRatio;

//             // If too wide, scale to fit width
//             let finalWidth = targetWidth;
//             let finalHeight = targetHeight;
//             if (targetWidth > rendererWidth * 0.9) {
//                 finalWidth = rendererWidth * 0.9;
...
//             // Position at center of renderer (not host element)
//             mannequin.x = rendererWidth / 2;
//             mannequin.y = rendererHeight / 2;
//             mannequin.zIndex = 100;
//             stage.addChild(mannequin);

//             // Update anchor conversion functions with correct dimensions
...
//                     const { cx, cy, pw, ph } = boxToStage(b); // Use the corrected function

//                     g.setStrokeStyle({ width: 2, color, alpha: 0.9 });
//                     g.rect(cx - pw / 2, cy - ph / 2, pw, ph); // Draw centered on the anchor point
//                     g.stroke();
//                 };
...
//             const fromStageNx = (sx: number) => ((sx - mannequin.x) / manW) + 0.5;
//             const fromStageNy = (sy: number) => ((sy - mannequin.y) / manH) + 0.5;
...
//                     // Labels (0, 0.5, 1) along axes - CORRECTED POSITIONS
...
//                 // Crosshair + tooltip - CORRECTED BOUNDS CHECK
...
//                     // Only show crosshair when inside mannequin bounds - CORRECTED BOUNDS
//                     const inside =
...
export default function TryOnCanvas({
    ...
    useEffect(() => {
        ...
        // Function to handle resize and repositioning
        const handleResize = () => {
            if (!app || !mannequin || !hostEl || destroyed) return;

            const containerWidth = hostEl.clientWidth;
            const containerHeight = hostEl.clientHeight;
            const hostElRect = hostEl.getBoundingClientRect();

            // console.log('Resize details:', {
            //     containerSize: `${containerWidth}x${containerHeight}`,
            //     boundingClientRect: `${hostElRect.width}x${hostElRect.height}`,
            //     hostElOffset: `${hostEl.offsetWidth}x${hostEl.offsetHeight}`,
            //     windowInner: `${window.innerWidth}x${window.innerHeight}`,
            //     mannequinPos: mannequin ? `${mannequin.x}, ${mannequin.y}` : 'null'
            // });

            // Resize the renderer to match container
            app.renderer.resize(containerWidth, containerHeight);

            // Re-center the mannequin
            mannequin.x = containerWidth / 2;
            mannequin.y = containerHeight / 2;

            // Recalculate scale based on new container size
            ...
            // console.log('Resized to:', containerWidth, containerHeight, 'Mannequin at:', mannequin.x, mannequin.y);
        };
        ...
        (async () => {
            ...
            // Set initial size based on host element
            const containerWidth = hostEl.clientWidth;
            const containerHeight = hostEl.clientHeight;
            ...
            // Force a resize after a short delay to ensure modal is fully open
    ...
    return <div ref={hostRef} className="try-on-container" style={{
        width: "100%",
        height: "100%",
        position: "relative",
